...
DATABASE_URL = os.getenv("DATABASE_URL", "postgresql+asyncpg://postgres:password@localhost:5432/localkart")
...
async def seed():
    ...
    async with AsyncSessionLocal() as db:
        # Users
        users = [
            User(name="Admin User",      email="admin@localkart.com",    hashed_password=hash_password("admin123"),    role=UserRole.admin),
            User(name="Ramesh Sharma",   email="shop@localkart.com",     hashed_password=hash_password("shop123"),     role=UserRole.shopkeeper),
            User(name="Priya Customer",  email="customer@localkart.com", hashed_password=hash_password("cust123"),     role=UserRole.customer),
            User(name="Delivery Rahul",  email="delivery@localkart.com", hashed_password=hash_password("delivery123"), role=UserRole.delivery),
        ]
        for u in users:
            db.add(u)
        await db.flush()
